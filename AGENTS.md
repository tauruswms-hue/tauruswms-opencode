...
Seed users (from `ADMIN_SEEDS` in `modules/schema_generator.py`): `admin` and `operador`, both password `Admin@2024!`. Los tests de integración **no** dependen de estos seeds: `tests/conftest.py` expone fixtures `usuario_wms` (en `taurus_admin.usuarios`) y `usuario_panel` (en `taurus_admin.admin_usuarios`) que crean usuarios temporales con password propio y los eliminan al final.
